...
TEXT = base64.b64encode(bytes("workflow bridge sale", 'utf-8'))
...
class TestCaseDocumentsBridgeSale(TransactionCase):
    ...
    def setUp(self):
        ...
        self.workflow_rule_task = self.env['documents.workflow.rule'].create({
            ...
            'name': 'workflow rule create sale on f_a',
    ...
    def test_bridge_folder_workflow(self):
        ...
        self.assertEqual(self.attachment_txt.res_model, 'sale.order', "failed at workflow_bridge_documents_project"
                                                                        " new res_model")
        sale_order = self.env['sale.order'].search([('id', '=', self.attachment_txt.res_id)])
        self.assertTrue(sale_order.exists(), 'failed at workflow_bridge_documents_project Sale Order')
        self.assertEqual(self.attachment_txt.res_id, sale_order.id, "failed at workflow_bridge_documents_sale_order res_id")
